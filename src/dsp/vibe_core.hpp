...
static inline void sanitize_user_params(VibeUserParams *params) {
    ...
    params->stereo_width = clampf(params->stereo_width, 0.0f, 1.35f);
...
static VibePreset make_vibe_preset(VibeVoicing voicing) {
    ...
    switch (voicing) {
        ...
        case VibeVoicing::ModernWide:
            ...
            preset.musical.mono_compat = 0.84f;
            preset.feedback_profile = FeedbackProfile::ModernFeedback;
        ...
        case VibeVoicing::WideStereoDream:
            ...
            preset.musical.mono_compat = 0.88f;
            preset.lfo_shape = LfoShape::Sine;
            break;
        ...
        case VibeVoicing::FastRotaryVibe:
            ...
            preset.musical.mono_compat = 0.72f;
            preset.lfo_shape = LfoShape::Sine;
            break;
        case VibeVoicing::BassSynthFriendly:
            ...
            preset.musical.mono_compat = 0.86f;
            preset.lfo_shape = LfoShape::Sine;
            break;
        ...
        case VibeVoicing::ModernHiFiPhaseVibe:
            ...
            preset.musical.mono_compat = 0.78f;
            preset.lfo_shape = LfoShape::Sine;
            break;
        ...
        default:
            preset.profile = VibeProfile::Classic;
            preset.chorus_mode = true;
            preset.user.mix = 0.52f;
            preset.user.depth = 0.86f;
            preset.user.feedback = 0.18f;
            preset.user.input_drive = 1.55f;
            preset.user.sat_asymmetry = 0.035f;
            preset.user.sat_out_trim = 0.96f;
            preset.user.lfo_rate_hz = 0.85f;
            preset.user.tone_tilt = 0.0f;
    ...
    preset.user.stereo_width = preset.musical.stereo_width;
...
void Vibe::update_time_constants() {
    const float lag = clampf(smoothed_user.lamp_lag, 0.35f, 2.50f);
    const float attack_sec = clampf(params.tuning.lamp_attack_sec * lag, 0.001f, 0.250f);
    const float release_sec = clampf(params.tuning.lamp_release_sec * lag, 0.001f, 0.500f);
...
void Vibe::out(float *smpsl, float *smpsr) {
    ...
    for (int i = 0; i < PERIOD; i++) {
        ...
        const float wet_makeup = clampf(1.0f / fmaxf(0.25f, params.tuning.bjt_gain_trim * smoothed_user.sat_out_trim), 1.0f, 3.6f);
        const float emitterfb_l = params.legacy_saturation ? clampf(params.tuning.emitter_fb_scale / mod_res_l, params.tuning.emitter_fb_min, params.tuning.emitter_fb_max) : 0.0f;
        const float emitterfb_r = params.legacy_saturation ? clampf(params.tuning.emitter_fb_scale / mod_res_r, params.tuning.emitter_fb_min, params.tuning.emitter_fb_max) : 0.0f;
        ...
        if (params.legacy_saturation) {
            for (int j = 0; j < 4; j++) {
                float cvolt = vibefilter(wet_phase_l, &stage[j].ecvc) +
                              vibefilter(wet_phase_l + emitterfb_l * stage[j].oldcvolt, &stage[j].vc);
        ...
        if (params.legacy_saturation) {
            for (int j = 4; j < 8; j++) {
                float cvolt = vibefilter(wet_phase_r, &stage[j].ecvc) +
                              vibefilter(wet_phase_r + emitterfb_r * stage[j].oldcvolt, &stage[j].vc);
                cvolt = clampf(cvolt, -stage_limit, stage_limit);
                float ocvolt = clampf(vibefilter(cvolt, &stage[j].vcvo), -stage_limit, stage_limit);
                stage[j].oldcvolt = ocvolt;
        ...
        const float wet_air_r = tone_tilt_process(wet_core_r, tone_tilt, tone_lp_r);
